...
def delete_in_batches(
    qs, *, batch_size: int = 1000, pk: str = "id", order_by: str = None, pre_delete=None, post_delete=None
) -> int:
    """
    ...
    If `order_by` names an indexed field, each batch resumes from where the last ended, rather than rescanning index
    entries of rows already deleted but not yet vacuumed - which makes large deletes quadratic.
    """
    ...
    while True:
        if order_by:
            batch_qs = qs.filter(**{f"{order_by}__gte": resume_from}) if resume_from is not None else qs
            rows = list(batch_qs.order_by(order_by).values_list(pk, order_by)[:batch_size])
            pk_batch = [r[0] for r in rows]
        else:
        ...
        if order_by:
            resume_from = rows[-1][1]  # inclusive as other rows may share this value
